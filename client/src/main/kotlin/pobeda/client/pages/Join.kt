        ...
        if (state.formSent) {
            ...
            styledForm(action = "", method = FormMethod.post) {
                styledP {
                    ...
                    + "При возникновении проблем с загрузкой и отправкой работы:\n"
                    + "1. Проверьте, все ли обязательные поля заполнены. Необязательными являются только позиции «есть куратор» и «добавить эссе»\n"
                    + "2. Уберите галочки с незаполненных необязательных полей «есть куратор» и «добавить эссе»\n"
                    + "3. В графе «возраст» указываете только цифра без «лет»\n"
                    + "4. Проверьте формат файла вашего рисунка. Он должен быть: jpg или png\n"
                    + "5. Если вышеуказанные действия не привели к результату, попробуйте зайти на сайт с другого браузера (Яндекс, Гугл) или с другого устройства.\n"
                    + "Сертификат приходит автоматически, сразу после загрузки на указанную почту.\n"
                    + "Если сертификат не пришел или на нем отразилась ошибка, допущенная при входе данных, заново загрузите работу, указав правильные данные.\n"
                    + "Если проблему нем удалось решить самостоятельно, обратитесь в службу поддержки сайта, указав возникшую проблему и приложив скрин экрана: uhatov007@yandex.ru"
                }
                css {
